...
class ProfileView(View):
    ...
    def get(self, request):
        ...
        if profile.banklink == False:
            ...
            profile.txnid=txnid
            profile.save()
            payload={"ver":"1.0","timestamp":datetime.datetime.utcnow().strftime('%Y-%m-%dT%H:%M:%S.%f')[:-3] + 'Z',
                     "txnid":txnid,
            ...
            payload={"ver":"1.0","timestamp":datetime.datetime.utcnow().strftime('%Y-%m-%dT%H:%M:%S.%f')[:-3] + 'Z',
                     "txnid":txnid,
                     "FIPDetails":{"fipId":"BARB0KIMXXX","fipName":"Bank of Baroda"},
                     "Customer":{"id":request.user.username+'@finvu',"Accounts":[{"maskedAccNumber":first_account['maskedAccNumber']
                                                                                     ,"accRefNumber":first_account['accRefNumber'],"FIType":first_account['FIType'],"accType":first_account['accType']}]}}
...
def send_wss(ws,req_type,payload,mid,sid=None):
    ...
    msg={"header":{"mid": str(uuid.uuid4()),"ts":datetime.datetime.utcnow().strftime('%Y-%m-%dT%H:%M:%S.%f')[:-3] + 'Z',"sid":sid,"dup":False,
